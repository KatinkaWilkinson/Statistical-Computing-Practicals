...
## Question 3
The standard deviation that is "NA" is linked to carrier "OO". This is because there is only one data point left once filtering for carrier "OO" in month 1.
```{r}
flights |> filter(carrier == "OO" & month == "1") |> nrow()
```
The standard deviation of 0 comes from carrier "YV". Filtering for "YV" in month 1 shows that all of the entries have a distance of 229, causing the standard deviation to be zero.

```{r}
flights |> filter(carrier == "YV", month == "1") |> nrow()
flights |> filter(carrier == "YV", month == "1") |> distinct(distance)
```
## Question 4

```{r}
flights |> group_by(carrier, month) |> summarise(avg_dep_delay = mean(dep_delay, na.rm = TRUE))|> select(month, carrier, avg_dep_delay) |> pivot_wider(names_from = carrier,
...
filtered_flights <- (flights |> filter(dep_delay>0 &  arr_delay<=0) |> nrow())
filtered_flights/total_flights
```
## Question 6

```{r}
routes <- flights |> group_by(origin, dest) |> summarise(num_airlines = n_distinct(carrier)) |> filter(num_airlines > 1)
routes
```
```{r}
carrier_delays <- flights |> inner_join(routes) |> group_by(carrier, origin, dest) |> summarise(average_delay = mean(arr_delay, na.rm = TRUE))
carrier_delays

av_delays_per_airline <- airlines |> left_join(carrier_delays) |> inner_join(carrier_delays)

av_delays_per_airline
```

For each such route, identify the airline with the worst and best average arrival delay.

```{r}
best_and_worst <- av_delays_per_airline |> group_by(origin, dest) |> summarise(best_average_delay = min(average_delay), best_airline = name[average_delay == min(average_delay)], worst_average_delay = max(average_delay), worst_airline = name[average_delay == max(average_delay)])
best_and_worst
```

```{r}
diff_bet_max_min <- best_and_worst |> mutate(difference = worst_average_delay - best_average_delay) 
diff_bet_max_min |> arrange(desc(difference))
```
Or we can extract the route with the greatest difference between the best and worst performing airlines using:

```{r}
diff_bet_max_min |>
  dplyr::ungroup() |>
  dplyr::filter(difference == max(difference, na.rm = TRUE))
```
To identify the reason, we can filter out all entries in the original table for that route:

```{r}
count_num_flights <- flights |> group_by(origin, dest) |> summarise(num_flights = n())
mean_num_flights <- mean(count_num_flights$num_flights)
mean_num_flights

num_flights_JFK_ATL <- count_num_flights |> filter(origin == "JFK", dest == "ATL") |> select(num_flights)
num_flights_JFK_ATL

max_num_flights <- count_num_flights |> arrange(num_flights)
max_num_flights
```
...
This shows us that there are NA values present in the height and glucose columns. There are also entries that have typos in the gender ("femal" instead of "female") and disease_status ("Healthy" instead of "healthy") columns. We can extract these colmns using the following commands:

```{r}
data |> filter(is.na(height) | is.na(glucose) | gender == "femal" | disease_status == "Healthy")
```